...
$logs = "C:\Users\Frask\Documents\cls\examples\audit\fase-1\_logs"
New-Item -ItemType Directory -Force -Path $logs | Out-Null

$script = (Resolve-Path $Script).Path
$base = [System.IO.Path]::GetFileNameWithoutExtension($script)

$outF = Join-Path $env:TEMP ("fase1_{0}_out.txt" -f $base)
$errF = Join-Path $env:TEMP ("fase1_{0}_err.txt" -f $base)
