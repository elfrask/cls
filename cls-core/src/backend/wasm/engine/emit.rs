...
impl<'a> Engine<'a> {
    ...
    pub(crate) fn emit(&mut self, module: &Module) -> ClsResult<Vec<u8>> {
        ...
        for h in [
            ...
            Sleep,
            Trap,
            // Nota: ParseInt/Float/Bool, Str*, Int/FloatAbs, PowNum, Fmod,
            // Arr*, Math* (salvo MathRandom), Record* y ArrToString ya NO se
            // importan del host: el emisor los llama a las internals `__intr_*`
            // fusionadas (paso 3). El fallback al host call de esos paths quedó
            // muerto (la fusión siempre está activa).
            Input,
            MathRandom,
            JsonStringify,
            JsonParse,
