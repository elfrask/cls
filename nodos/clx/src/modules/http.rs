...
pub fn module() -> Value {
    ...
    m.insert("get".into(), Value::Fun(FunValue::new_native("get", vec!["url".into()], |a| {
        ...
        let resp = ureq::get(&url).call().map_err(|e| ClsError::RuntimeError(format!("http.get: {}", e)))?;
        let body = resp.into_string().map_err(|e| ClsError::RuntimeError(format!("http.get: {}", e)))?;
    ...
    m.insert("post".into(), Value::Fun(FunValue::new_native("post", vec!["url".into(), "body".into()], |a| {
        ...
        let resp = ureq::post(&url).send_string(&body_content).map_err(|e| ClsError::RuntimeError(format!("http.post: {}", e)))?;
        let body = resp.into_string().map_err(|e| ClsError::RuntimeError(format!("http.post: {}", e)))?;
        Ok(Value::String(body))
    })));
    Value::Record(m)
}
